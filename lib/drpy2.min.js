...
function __codexLoad(html) {
    return cheerio.load(typeof html === "string" ? html : String(html || ""))
}
function __codexPick($, parse) {
    parse = (parse || "").trim();
    let parts = parse.split("&&").map(it => it.trim());
    let attr = parts.length > 1 ? parts.pop() : "Text";
    let sel = parts.join("&&").trim();
    let node = sel ? $(sel).first() : $.root();
    if (/^(Text|text)$/i.test(attr)) return node.text();
    if (/^(Html|html)$/i.test(attr)) return node.html() || "";
    if (/^(OuterHtml|outerHtml)$/i.test(attr)) return $.html(node);
    return node.attr(attr) || ""
}
const defaultParser = {
    pdfh(html, parse) {
        if (!html || !parse) return "";
        let $ = __codexLoad(html);
        return __codexPick($, parse)
    },
    pdfa(html, parse) {
        if (!html || !parse) return [];
        let $ = __codexLoad(html);
        return $(parse.trim()).toArray().map(el => $.html(el))
    },
    pd(html, parse, base_url) {
        let ret = this.pdfh(html, parse);
        return ret ? urljoin(base_url || MY_URL, ret) : ret
    }
};
